function [imagingData, BehaveData] = load_data_files(datapath)
...
if ~isfolder(datapath)
    error([datapath ' is not a folder']);
end

bdalist = dir(fullfile(datapath, 'BDA_TSeries_*.mat'));
if isempty(bdalist)
    warning(['no bda for ' datapath]);
end
tpalist = dir(fullfile(datapath, 'TPA*.mat'));
if isempty(tpalist)
    error(['no tpa for ' datapath']);
    
end
trialindBDA=[];
for l = 1:length(bdalist)
    si = strfind(bdalist(l).name, '_Cycle');
    trialindBDA(l) = str2double(bdalist(l).name(si-3:si-1));
end
trialindTPA = [];
for l = 1:length(tpalist)
    si = strfind(tpalist(l).name, '_Cycle');
    trialindTPA(l) = str2double(tpalist(l).name(si-3:si-1));
end
