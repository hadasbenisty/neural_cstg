function plot_flavors_svm_by_cell_summary_q_vs_all(datapath, resultspath, animals_names, figspath, excelpath)
...
for animal_i = 1:length(animals_names)
    ...
    resfileall = fullfile(resultspath, ['svm_cells_' animals_names{animal_i} '_q_vs_all.mat']);
    if 0&&isfile(resfileall)
...
%         sel = find(strcmp(explabel(sum(isnan(x))~=minNrns), 'fi ba'));
%         [~, ic] = sort(yy(:, sel(end)));
...
%     mysave(gcf, fullfile(figspath, 'per_animal_stats', ['svm_cells_' animals_names{animal_i} '_q_vs_all']));
    t1 = unique(type1);
    t2 = unique(type2);
...
for n1 = 1:length(t1)-1
    for n2 = 1:length(t2)
        ...
        barwitherr(100*Sfrac(:, n1, n2)/sqrt(Nfrac(n1, n2)-1), ...
            100*Mfrac(:, n1, n2));
 ...
 mysave(gcf,fullfile(figspath, 'across_animals_stats',...
            ['svm_single_cells_indicative_'  '_' ttl2]))
...
resfile = fullfile(resultspath, ['svm_by_cell_' animal '_' folder '_q_vs_all.mat']);
if ~isfile(resfile)
    return;
end
...
accall = acc.mean - acc.chance;
SEM = acc.std/sqrt(acc.trialsnum-1);               % Standard Error
ts = tinv(1-2*pvalue, (acc.trialsnum)-1);      % T-Score
isindicative = acc.mean-ts*SEM > acc.chance;
for win_i = 1:size(wins, 1)
    
     x = isindicative(1:Nnrns, ...
        tmid>=wins(win_i, 1) & tmid<= wins(win_i, 2));
    
    indic_by_win(:, win_i) = mean(x > 0, 2)>0.1;
    del_acc_mean(:, win_i) = mean(acc.mean(1:Nnrns, ...
        tmid>=wins(win_i, 1) & tmid<= wins(win_i, 2)), 2) - acc.chance;
end
