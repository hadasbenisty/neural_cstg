...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        hErrorbar(col) = myerrorbar(mean(x,1), values(xOrder,col), lowerErrors(xOrder,col), upperErrors(xOrder, col), 'black');
%         set(hErrorbar(col), 'marker', 'none')
    ...
    hErrorbar = myerrorbar(mean(x,1), values, lowerErrors, upperErrors, '.k');
