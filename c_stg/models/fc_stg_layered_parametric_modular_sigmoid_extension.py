...
__all__ = ['FC_STG_Layered_Param_modular_model_sigmoid_extension',
           'fc_stg_layered_param_modular_model_sigmoid_extension']


class FeatureSelector(nn.Module):
    def __init__(self, hyper_input_dim, hyper_output_dim, hyper_hidden_dim, sigma=0.5, non_param_stg=False,
                 train_sigma=False, is_cen=False):
...
class FC_STG_Layered_Param_modular_model_sigmoid_extension(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, param_dim, hyper_hidden_dim, dropout, sigma, stg,
                 classification, include_B_in_input=False, non_param_stg=False, train_sigma=False, is_cen=False):
        super(FC_STG_Layered_Param_modular_model_sigmoid_extension, self).__init__()
        self.sigma = sigma  # shira
        ...
        if classification:
            ...
            # the cross entropy of pytorch already include softmax, so the last layer should be linear, shira
            # else:
...
def fc_stg_layered_param_modular_model_sigmoid_extension(input_dim, hidden_dim=[10], output_dim=1, param_dim=1,
                                                         hyper_hidden_dim=[500], dropout=0, sigma=0.5, stg=True,
                                                         classification=True, include_B_in_input=False,
                                                         non_param_stg=False, train_sigma=False):
    model = FC_STG_Layered_Param_modular_model_sigmoid_extension(input_dim, hidden_dim, output_dim, param_dim,
